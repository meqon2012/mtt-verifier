...
async function analyze() {
    ...
    try {
        ...
        Object.keys(fileContents).forEach(key => {
            const content = fileContents[key].substring(0, 3000); // Увеличили до 3000 символов
            filesText += '\n=== Файл: ' + key + ' ===\n' + content + '\n';
        });
        ...
        // Отправляем на backend с содержимым файлов
        const response = await fetch('/api/analyze', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ 
                prompt: promptText,
                files: Object.keys(fileContents).map(key => ({
                    name: key,
                    content: fileContents[key].substring(0, 5000)
                })),
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.error || 'API ошибка: ' + response.status);
        }
        ...
        showSuccess('✅ Ответ получен от ИИ');
...
function displayResults(content) {
    ...
    const html = `
        <div class="ai-response-box">
            <div class="ai-response-label">🤖 Ответ от Perplexity AI:</div>
