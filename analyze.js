// ==========================================
// ANALYZE.JS - ИСПРАВЛЕННАЯ ВЕРСИЯ
// Правильно отправляет содержимое PDF и других файлов в ИИ
// ==========================================
...
// ==========================================
// АНАЛИЗ ДОКУМЕНТОВ - ГЛАВНОЕ ИСПРАВЛЕНИЕ!
// ==========================================
...
async function analyze() {
    ...
    try {
        // ✅ ИСПРАВЛЕНИЕ #1: Проверяем что fileContents не пустой
        const contentKeys = Object.keys(fileContents);
        console.log('📄 Загруженные файлы:', uploadedFiles.length);
        console.log('📋 Файлы с содержимым:', contentKeys);
        
        if (contentKeys.length === 0) {
            showError('⚠️ Внимание: содержимое файлов не загружено. Пожалуйста, подождите загрузки.');
            loading.classList.remove('active');
            return;
        }

        // ✅ ИСПРАВЛЕНИЕ #2: Правильно формируем текст файлов
        let filesText = '';
        contentKeys.forEach(key => {
            // Берем ДО 5000 символов из каждого файла (был 3000)
        ...
        console.log('📤 Размер содержимого файлов:', filesText.length, 'символов');

        // ✅ ИСПРАВЛЕНИЕ #3: Берем промт и ПРАВИЛЬНО подставляем файлы
        let promptText = prompts[currentPromptId].text;
        
        // Проверяем что в промте есть placeholder {files}
        if (promptText.includes('{files}')) {
            promptText = promptText.replace('{files}', filesText);
        } else {
            // Если нет - добавляем файлы в конец
            promptText = promptText + '\n\nФайлы контрагента:\n' + filesText;
        }

        console.log('📤 Отправляю запрос в Perplexity API...');
        console.log('Используемая модель:', MODEL);
        console.log('Итоговый размер промта:', promptText.length, 'символов');
        console.log('Первые 500 символов промта:', promptText.substring(0, 500));

        // ✅ ИСПРАВЛЕНИЕ #4: Отправляем в API с правильным промтом
        const response = await fetch('https://api.perplexity.ai/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'user',
                        content: promptText  // ✅ ЗДЕСЬ ОТПРАВЛЯЕТСЯ ПОЛНЫЙ ПРОМТ С СОДЕРЖИМЫМ ФАЙЛОВ
                    }
                ],
                temperature: 0.2,
                top_p: 0.9
        ...
        console.log('✅ Содержимое ответа получено, длина:', rawContent.length);

        displayResults(rawContent);
        showSuccess('✅ Ответ получен от Perplexity AI (файлы обработаны)');
